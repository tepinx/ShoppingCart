...
public class ShoppingCart {
	...
	public Boolean add(
		...
		if (itemsAdded != null) {
			...
			itemsAdded.setOrigPriceTotal(formatAmount(itemsAdded.getOrigPriceTotal().add(itemsAdded.getItem().getPrice())));
		} else {
			// put new items added in the map
			cart.getItemsAddedMap().put(item.getCode(), new ItemsAdded(item, 1, formatAmount(item.getPrice())));
		}
	...
	public BigDecimal total() {
		...
		return formatAmount(cart.getTotal());
	}
	...
	private void applyPricingRule(
		...
		if (cart.isPricingRulesApplied() == false && cart.getPricingRules() != null) {
			for (PricingRule pr : cart.getPricingRules()) {
				if (pr instanceof AddPromoCodeRuleImpl) {
					...
					pr.applyRule(cart);
					cart.setPricingRulesApplied(true);
	 ...
	 * @return BigDecimal
	 */
	private BigDecimal formatAmount(BigDecimal price) {
